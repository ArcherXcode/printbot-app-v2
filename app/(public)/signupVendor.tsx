...
const businessSchema = z.object({
  ...
  email_legal: z.string().min(1, 'Legal email is required').email('Enter a valid legal email'),
  email_contact: z.string().min(1, 'Contact email is required').email('Enter a valid contact email'),
  phone_legal: z.string().min(10, 'Enter a valid legal phone number'),
  phone_contact: z.string().min(10, 'Enter a valid contact phone number'),
});

const bankingSchema = z.object({
  bank_ifsc_code: z.string().min(1, 'IFSC code is required'),
  ...
  bank_branch_address: z.string().min(1, 'Branch address is required'),
});
...
────────────────────────────────────────── */
const STEPS = [
  { title: 'Identity', description: 'Owner identity and account login' },
  { title: 'Address', description: 'Physical shop address' },
  { title: 'Business', description: 'Legal and operational contact profile' },
  { title: 'Banking', description: 'Settlement account configuration' },
  { title: 'Legal', description: 'Regulatory verification' },
  { title: 'Picture', description: 'Upload profile image' },
  { title: 'Review', description: 'Confirm and submit onboarding' },
];
...
export default function SignupVendorScreen() {
  ...
  const signupMutation = useSignupVendorMutation();
  ...
  const [form, setForm] = useState<FormData>({
    ...
    business_legal_name: '', email_legal: '', email_contact: '',
    phone_legal: '', phone_contact: '',
    bank_ifsc_code: '', bank_swift_code: '', bank_account_number: '',
    bank_account_holder_name: '', bank_branch_address: '',
    gstin: '', pan_number: '',
  });
  ...
  const validateStep = (): boolean => {
    ...
    if (currentStep === 0) result = identitySchema.safeParse(form);
    else if (currentStep === 1) result = addressSchema.safeParse(form);
    else if (currentStep === 2) result = businessSchema.safeParse(form);
    else if (currentStep === 3) result = bankingSchema.safeParse(form);
    else if (currentStep === 4) result = legalSchema.safeParse(form);
    else return true;
  ...
  const handleSubmit = () => {
    ...
    const payload: SignupVendorPayload = {
      ...rest,
      latitude: '0',
      longitude: '0',
    };
          ...
          <View key={step.title} style={styles.stepItem}>
            <View style={[
              ...
              {isDone ? (
                ...
                <Text style={[
                  styles.stepNumber,
                  { color: isActive ? colors.stepActive : colors.stepTextInactive },
                ]}>
                  {index + 1}
                </Text>
              )}
  ...
  const StepContent = () => {
    ...
    if (currentStep === 1) return (
      <View style={styles.fieldsGrid}>
        ...
        <Field label="Address Line 1 *" value={form.address_line_1} onChangeText={set('address_line_1')}
          placeholder="Apartment, suite, floor…" autoCapitalize="words"
          error={errors.address_line_1} colors={colors} />
        <Field label="Address Line 2" value={form.address_line_2} onChangeText={set('address_line_2')}
          placeholder="(optional)" autoCapitalize="words"
          error={errors.address_line_2} colors={colors} />
    ...
    if (currentStep === 2) return (
      <View style={styles.fieldsGrid}>
        <Field label="Business Legal Name *" value={form.business_legal_name}
          onChangeText={set('business_legal_name')} placeholder="Acme Print Pvt Ltd"
          autoCapitalize="words" error={errors.business_legal_name} colors={colors} />
        <Field label="Legal Email *" value={form.email_legal} onChangeText={set('email_legal')}
          placeholder="legal@acme.com" keyboardType="email-address" autoComplete="email"
          error={errors.email_legal} colors={colors} />
        <Field label="Contact Email *" value={form.email_contact} onChangeText={set('email_contact')}
          placeholder="contact@acme.com" keyboardType="email-address" autoComplete="email"
          error={errors.email_contact} colors={colors} />
        <Field label="Legal Phone *" value={form.phone_legal} onChangeText={set('phone_legal')}
          placeholder="+91 99999 99999" keyboardType="phone-pad" autoComplete="tel"
          error={errors.phone_legal} colors={colors} />
        <Field label="Contact Phone *" value={form.phone_contact} onChangeText={set('phone_contact')}
          placeholder="+91 88888 88888" keyboardType="phone-pad" autoComplete="tel"
          error={errors.phone_contact} colors={colors} />
      </View>
    );
    ...
    if (currentStep === 3) return (
      <View style={styles.fieldsGrid}>
        <Field label="Bank IFSC *" value={form.bank_ifsc_code} onChangeText={set('bank_ifsc_code')}
          placeholder="HDFC0001234" autoCapitalize="characters"
          error={errors.bank_ifsc_code} colors={colors} />
        <Field label="Bank SWIFT" value={form.bank_swift_code} onChangeText={set('bank_swift_code')}
          placeholder="HDFCINBB (optional)" autoCapitalize="characters"
          error={errors.bank_swift_code} colors={colors} />
        <Field label="Account Number *" value={form.bank_account_number}
          onChangeText={set('bank_account_number')} placeholder="XXXXXXXXXXXX"
          keyboardType="number-pad" error={errors.bank_account_number} colors={colors} />
        <Field label="Account Holder Name *" value={form.bank_account_holder_name}
          onChangeText={set('bank_account_holder_name')} placeholder="John Doe"
          autoCapitalize="words" error={errors.bank_account_holder_name} colors={colors} />
        <Field label="Branch Address *" value={form.bank_branch_address}
          onChangeText={set('bank_branch_address')} placeholder="123 Bank Street, Mumbai"
          autoCapitalize="words" error={errors.bank_branch_address} colors={colors} />
      </View>
    );
    ...
    if (currentStep === 4) return (
      <View style={styles.fieldsGrid}>
        <Field label="GSTIN *" value={form.gstin} onChangeText={set('gstin')}
          placeholder="22AAAAA0000A1Z5" autoCapitalize="characters"
          error={errors.gstin} colors={colors} />
        <Field label="PAN Number *" value={form.pan_number} onChangeText={set('pan_number')}
          placeholder="ABCDE1234F" autoCapitalize="characters"
          error={errors.pan_number} colors={colors} />
        <View style={[styles.infoNote, { backgroundColor: colors.input, borderColor: colors.inputBorder }]}>
          <Feather name="shield" size={14} color={colors.icon} style={{ marginRight: 8 }} />
          <Text style={[styles.infoNoteText, { color: colors.subheading }]}>
            Your tax details are encrypted and used only for regulatory legal. They will never be shared.
          </Text>
        </View>
      </View>
    );
      ...
      <View style={styles.photoStep}>
        ...
        <View style={[styles.photoNote, { backgroundColor: colors.input, borderColor: colors.inputBorder }]}>
          ...
          <Text style={[styles.photoNoteText, { color: colors.subheading }]}>
            Photo upload from the mobile app is coming soon. You can upload via the web portal in the meantime.
          </Text>
        </View>
      </View>
    );
    ...
    if (currentStep === 6) {
      const reviewGroups = [
        ...
        },
        {
          title: 'Address',
          rows: [
            { label: 'Address', value: [form.address, form.address_line_1, form.address_line_2].filter(Boolean).join(', ') },
        ...
        },
        {
          title: 'Business',
          rows: [
            { label: 'Legal Name', value: form.business_legal_name },
            { label: 'Legal Email', value: form.email_legal },
            { label: 'Contact Email', value: form.email_contact },
            { label: 'Legal Phone', value: form.phone_legal },
            { label: 'Contact Phone', value: form.phone_contact },
          ],
        },
        {
          title: 'Banking',
          rows: [
            { label: 'IFSC', value: form.bank_ifsc_code },
            { label: 'SWIFT', value: form.bank_swift_code || '—' },
            { label: 'Account No.', value: form.bank_account_number ? `****${form.bank_account_number.slice(-4)}` : '—' },
            { label: 'Holder Name', value: form.bank_account_holder_name },
          ],
        },
        {
          title: 'Legal',
        ...
        <View style={styles.reviewContainer}>
          ...
          <View style={[styles.reviewNote, { backgroundColor: 'rgba(89,134,231,0.08)', borderColor: '#5986e7' }]}>
            ...
            <Text style={[styles.reviewNoteText, { color: '#5986e7' }]}>
              Ready to onboard your vendor account. Tap Create Account to submit.
            </Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  /* ── Step indicator — 7 steps need tighter circles ── */
  stepRow: {
    flexDirection: 'row',
  ...
  photoNoteText: { fontSize: 13, flex: 1, lineHeight: 18 },
